...
function Portfolio({ noHeader = false }) {
  return (
    <div id="our-portfolio" className="flex flex-col w-full max-w-[1300px] mx-auto mt-52  px-4">
      {!noHeader && (
        <>
        ...
        {[0, 3].map((start, rowIndex) => (
          ...
          >
            {portfolioItems.slice(start, start + 3).map((item, index) => (
              <div
                key={index}
                className={`flex flex-row border border-gray-400 bg-[#EDEDED] dark:bg-[#171717] rounded-[16px] h-auto min-h-[300px] ${item.width} relative items-center`}
              >
